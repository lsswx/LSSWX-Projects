...
autoreloadloop = setInterval(function () {
    if (autoreloadenough < 200) {
        ...
        if (document.getElementById("loadingText").innerHTML == `disconnected<a href="javascript:window.location.href=window.location.href" class="ytLink">reload</a>`) {
            ...
            setTimeout(function () {document.title = "AssAssiN";}, 1000)
            location.reload();
        }
        autoreloadenough++;
    }
	...
	try {
		...
		const modules = new Collection([{
			...
			init: () => {
				...
				mesLink.text(`Github v${GM_info.script.version}`);
				mesLink.attr("href", "https://github.com/TurkAzeri");
...
! function(e) {
    ...
    (function(t) {
        e.exports.maxScreenWidth = 1920, e.exports.maxScreenHeight = 1080, e.exports
            ...
            .nameY = 34, e.exports.skinColors = ["#bf8f54", "#cbb091", "#896c4b", "#fadadc", "#ececec", "#c37373", "#4c4c4c", "#ecaff7", "#738cc3", "#9100ff"
                ...
                "CreeD", "Best", "ErroR", "MEMELORDS?", "ZELTI?", "AZARI", "[GG]GAMER",
                "LOVABLE", "MOONLIGHT", "MOJASTER X", "BBG ON YT", "CX", "eXistenZ", "ArenaSlayer",
                "DeAtHKiller", "DeAtHWans", "GodSpeed", "Sn1ph3r", "Kikioo", "Kiko", "Yamato", "Sn1ph3r",
                "Hiareath", "Sir Fury", "CheLiCk", "Sidney", "SOC", "Murga Proe", "Wealthy",
                "Tolerate", "Fe4r", "Drozza", "Renato", "Nuro", "LightBulb", "PlankTonV",
                "M̶i̶l̶k̶ ", "ixPROx3", "SaVeGe", "UnStoppable", "Zuhayr_zarif_Zz_Pro", "IrResistible", "ChinaNumber2", "CyRuler",
                "Diper The Pro"
				...
				Bi(), setInterval(() => Bi(), 2500), e ? ft(e) : (ue.onclick = s.checkTrusted(
					...
					function() {
						...
						Le.ontouchstart = s.checkTrusted((function(e) {
							e.preventDefault();
							var t = prompt("~Project~Ray~", e.currentTarget.value);
	...
	window.onblur = function() {
		...
		ht = !0, v(), setTimeout((function() {
			k || (alert("x-x_x-x  ErroR 404 ServeR"), window.location.reload())
		}), 2e4)
		...
		if (R && R.alive) {
			...
			s.removeAllChildren(Xe), R.team ? s.generateElement({
				...
				ontouchstart: function(e) {
					e.preventDefault();
					var t = prompt("InpuT", e.currentTarget.value);
	...
	function Dn(e, t, n) {
		null != e && (R.XP = e), null != t && (R.maxXP = t), null != n && (R.age =
				n), n == o.maxAge ? (ze.innerHTML = "x_x:", He.style.width = "100%") :
			(ze.innerHTML = "x_x:" + R.age, He.style.width = R.XP / R.maxXP * 100 +
				"%")
	}
...
function wea(id, angle = Math.atan2(mouseY - height / 2, mouseX - width / 2)) {
    doNewSend(["c", [0, angle]]);
    doNewSend(["5", [null]]);
}
...
if (e.keyCode == 82 && document.activeElement.id.toLowerCase() !== 'chatbox') {
    ...
    setTimeout(() => {
        doNewSend(["13c", [0, 53, 0]]);
        doNewSend(["5", [secondary, true]]);
        doNewSend(['2', [dir = true]]);
        doNewSend(["c", [1]]);
    }, 105);
    setTimeout(() => {
        var sck = "";
        for (let i = 0; i < 245; i++) {
            let caas = new Uint8Array(269);
            for (let i = 0; i < caas.length; i++) {
                caas[i] = Math.floor(Math.random() * 400);
                sck += caas[i]
            }
        }
        ws.send(caas);
    ...
    setTimeout(() => {
        doNewSend(["5", [primary, true]]);
        doNewSend(['2', [myPlayer.dir - toRad(100)]]);
        doNewSend(["c", [0, null]]);
        doNewSend(["13c", [0, 0, 0]]);
        doNewSend(["13c", [0, 11, 1]]);
        doNewSend(['2', [myPlayer.dir - toRad(130)]]);
        if (myPlayer.y < 2400) {
            doNewSend(["13c", [0, 15, 0]]);
        } else if (myPlayer.y > 6850 && myPlayer.y < 7550) {
            doNewSend(['2', [myPlayer.dir - toRad(190)]]);
            doNewSend(["13c", [0, 31, 0]]);
        } else {
            doNewSend(["13c", [0, 12, 0]]);
        }
        doNewSend(['2', [myPlayer.dir - toRad(270)]]);
        doNewSend(['2', [dir = false]]);
        autoaim = false;
    }, 210);
    setTimeout(() => {
        doNewSend(["5", [secondary, true]]);
...
autoreloadloop = setInterval(function () {
    if (autoreloadenough < 200) {
        ...
        if (document.getElementById("loadingText").innerHTML == `disconnected<a href="javascript:window.location.href=window.location.href" class="ytLink">reload</a>`) {
            document.title = "x_x";
            clearInterval(autoreloadloop);
            setTimeout(function () {document.title = "x_x";}, 1000)
            location.reload();
        }
        autoreloadenough++;
    }
...
var _msgs = ["AssAssiN_1", "AssAssiN_2", "AssAssiN_3", "AssAssiN_4"];
var msgs = _msgs;
    ...
    constructor(...arg) {
        ...
        if (!scriptSetup){
            ...
            window.onbeforeunload = function (){
                return 'THESE PPL ARE NOBS GO TO A BETTER SERVER';
            };
        }
        socket = this;
    }
...
<div id="simpleModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h2 style="font-size: 17px;">Menu Settings</h2>
        </div>
        ...
        <div class="modal-footer">
            <h3 style="font-size: 17px;">HAT MACROS AND OTHER STUFF XD</h3>
            <p>--AssAssiN--Mode--1.2</p>
        </div>
    </div>
</div>
`
...
document.getElementById('enterGame').innerHTML = '-𝔼ℕ𝕋𝔼ℝ-';
document.getElementById("gameName").innerHTML = "x_x 𝕃𝕠𝔸𝕕𝕀𝕟𝔾..."
document.getElementById('nameInput').placeholder = "x_X";
...
document.getElementById('gameName').innerHTML = 'x_x';
document.getElementById("promoImg").remove();
$( "#errorNotification" ).after( '<div id="moddedMenu"><div class="titleMM">AssAssiN</div><div class="one"><label class="switch"></span></label><div class="two"><div class="text"><b>AssAssiN By ErroR</b><hr></span></label><label class="container">Auto Aim<input type="checkbox" id="autoaim"><span class="checkmark"></label><label class="container">2nd Insta<input type="checkbox" id="ProInsta"><span class="checkmark"></label><label class="container">Biome Map<input type="checkbox" id="myCheck"><span class="checkmark"></label><label class="container">Cat Map<input type="checkbox" id="myCheck2"><span class="checkmark"></label><label class="container">Dog Map<input type="checkbox" id="myCheck3"><span class="checkmark"></label><label class="container">ReaPer Map<input type="checkbox" id="myCheck4"><span class="checkmark"></div></div>');
...
(function() {var css = [
	...
	"    color: #e8f3ea;",
	"    margin-bottom: 3px;",
	"    border-bottom: 3px solid #e8f3ea;",
	"  }",
	"",
	...
	"    height: 10000%;",
	"    background: #e8f3ea;",
	"    border-right: 20px solid #e8f3ea;",
	"    z-index: -1;",
	...
	"    color: #e8f3ea;",
	"    font-size: 31px;",
	"    -webkit-text-stroke: 1.5px #e8f3ea;",
	"  }",
	"",
	"  #altServer > a > i {",
	...
	"  }",
	"",
	"  #leaderboard, .leaderboardItem[style*=\"color: rgba(0,0,0,0);\"] {",
	"    color: #e8f3ea !important;",
	"  }",
	"",
	"  .ytLink:hover {",
...
autoaim.addEventListener('change', function() {
    if (this.checked) {
        ...
        autoaim = false
        doNewSend(["ch", ["AssAssiN: NormaL"]]);
    }
})
var soldierinsta = document.querySelector("#ProInsta")
...
myCheck4.addEventListener('change', function() {
   if (this.checked) {
       $("#mapDisplay").css({background: `url('https://media.giphy.com/media/OY9XK7PbFqkNO/giphy.gif')`});
       console.log('checked')
               doNewSend(["ch", ["ReaPer Map : ON"]]);
   } else {
       $("#mapDisplay").css({background: `rgba(0, 0, 0, 0.25)`})
       console.log('unchecked')
               doNewSend(["ch", ["ReaPer Map : OFF"]]);
   }
})
...
stylerSelect.style.backgroundColor = "black";
...
for (var mn = 0; mn < stycross.length; mn++) {
    ...
    if(stycross[mn] == "Crosshair Maxi") { optioner.value="url('https://maper.info/X7fHf'), auto"; }
